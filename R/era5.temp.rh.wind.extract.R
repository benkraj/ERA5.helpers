...
era5.temp.rh.wind.extract <- function(latitude, longitude, date.start, date.end){
  ...
  df <- df %>%
    ...
    dplyr::select(seq.years, time, plot.night, t2m, rh10, tp, wd180, ws180) %>%
    ...
    mutate(latitude=latitude,
           longitude=longitude) %>%
    dplyr::select(plot.night, latitude, longitude, mean.t2m, mean.rh10, cum.tp, wd.average, scalar.mean.ws.180)
  return(df)
  closeAllConnections()
}